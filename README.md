...
- 🎯 **검증된 한국 점자 매핑** — 2017 규정 + 2024 개정 완전 구현 (단독 약자 11자 + VF 약자 13종). 사용자 실인쇄 검증 + reference 변환기와 cross-check 한 **58 케이스 round-trip 테스트** 통과 (`tests/smoke_braille.py`).
...
- 표준 (2017 한국 점자 규정) 명확히 있고 검증 도구도 있는데, AI 가 자기 기억에만 의존하면 곳곳에 작은 오타가 박힙니다. **신뢰할 수 있는 변환기와 cross-check** 필수.
- 메시 검증: `is_watertight=True` 만 보지 말고 **volume 부호** 와 winding 일관성도 확인.
- `replace_all=True` 는 변경 후 반드시 **두 callsite 모두** 결과 확인.
- 한국 점자 약자 = **단독 11자 (제29항) + VF 13종 (제29항)** 둘 다 챙기기.
